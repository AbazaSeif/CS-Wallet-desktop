...
public class ApiClient {
    ...
    private BigDecimal getBalanceInternal(byte[] address, byte currency)
        throws LevelDbClientException, CreditsNodeException {

        LOGGER.info(String.format("---> address = %s; currency = %s", Converter.byteArrayToString(address, null), currency));
        API.Client client;
        try {
    ...
    public BigDecimal getBalance(byte[] address, byte currency) throws LevelDbClientException, CreditsNodeException {
        ...
        return getBalanceInternal(address, currency);
    }
    ...
    public List<TransactionData> getTransactions(byte[] address, long offset, long limit)
        ...
        for (SealedTransaction sealedTransaction : result.getTransactions()) {
            transactionDataList.add(LevelDbClientConverter.transactionToTransactionData(sealedTransaction.getTrxn()));
        }
        return transactionDataList;
    }

    public TransactionData getTransaction(TransactionIdData transactionIdData) throws LevelDbClientException, CreditsNodeException {
        API.Client client;
        try {
            client = pool.getResource();
        } catch (ThriftClientPool.ThriftClientException e) {
        ...
        pool.returnResource(client);
        ApiClientUtils.processApiResponse(result.getStatus());
        return LevelDbClientConverter.transactionToTransactionData(result.getTransaction().getTrxn());
    }
    ...
    private ApiResponseData transactionFlowInternal(long innerId, byte[] source, byte[] target, BigDecimal amount,
        BigDecimal balance, byte currency, byte[] signature, BigDecimal fee)
            throws LevelDbClientException, CreditsNodeException {
        ...
        LOGGER.info(
            String.format("---> source = %s; target = %s; amount = %s; balance = %s; currency = %s; signature = %s; innerId = %s",
        ...
        Transaction transaction = new Transaction(innerId, ByteBuffer.wrap(source), ByteBuffer.wrap(target),
                serverAmount, serverBalance, currency, ByteBuffer.wrap(signature), serverFee);

        TransactionFlowResult result = null;
        try {
    ...
    /**
     * Smart Contract Deployment (Generation of a transaction with a smart contract)
     *
     * @param transactionInnerId
     * @param transactionSource
     * @param transactionTarget
     * @param smartContractInvocationData
     * @return
     * @throws LevelDbClientException
     * @throws CreditsNodeException
     */
    public ApiResponseData deploySmartContract(
            long transactionInnerId,
            byte[] transactionSource,
        ...
        SmartContractInvocation smartContractInvocation = LevelDbClientConverter.smartContractInvocationDataToSmartContractInvocation(smartContractInvocationData);
        ...
        Transaction transaction =
            new Transaction(transactionInnerId, ByteBuffer.wrap(transactionSource), ByteBuffer.wrap(transactionTarget), null, null, (byte)1, ByteBuffer.wrap(signature), null);
     ...
     * @throws LevelDbClientException
     */
    public ApiResponseData executeSmartContract(
            long transactionInnerId,
            byte[] transactionSource,
            byte[] transactionTarget,
            SmartContractInvocationData smartContractInvocationData,
        ...
        ApiClientUtils.logApiResponse(result.getStatus());
        ApiClientUtils.processApiResponse(result.getStatus());
        return LevelDbClientConverter.apiResponseToApiResponseData(
                result.getStatus(),
     ...
     *
     * @param transactionFlowData
     * @param checkBalance- bool parameter
     * @return
     * @throws LevelDbClientException
     * @throws CreditsNodeException
     */
    public ApiResponseData transactionFlow(TransactionFlowData transactionFlowData, boolean checkBalance)
        ...
        if (checkBalance) {
            BigDecimal balanceFromNode = getBalanceInternal(source, currency);
            if (balanceFromNode.compareTo(amount) < 0) {
                throw new LevelDbClientException(
                    String.format("Wallet %s with balance [%s %s] is less than transaction amount [%s %s]", Converter.encodeToBASE58(source),
                        Converter.toString(balanceFromNode), currency,
                        Converter.toString(amount), currency));
            }
        }

        return transactionFlowInternal(innerId, source, target, amount, balance, currency, signature, fee);
    }

    public SmartContractData getSmartContract(byte[] address) throws LevelDbClientException, CreditsNodeException {

        return getSmartContractInternal(address);
    }

    private SmartContractData getSmartContractInternal(byte[] address)
        throws LevelDbClientException, CreditsNodeException {
    ...
    public List<ByteBuffer> getSmartContractAddresses(byte[] address) throws LevelDbClientException, CreditsNodeException {
        LOGGER.info(String.format("---> wallet address = %s", Converter.byteArrayToString(address, null)));
        API.Client client;
        try {
            client = pool.getResource();
